@if(@Auth::user()->permissionsGroup->add_status)
    ...
    <script>
        $("#import_btn").click(function () {
            ...
            $("#from_row").val("0");
            $("#record-import .form-fields").html("<div class=\"text-center p-a-2\"><img class=\"m-b-1\" src=\"{{ asset('public/assets/dashboard/images/loading.gif') }}\" style=\"height: 35px;\"/><div class='text-muted small'>{{ __("backend.loading") }}</div></div>");
        ...
        $('#record-import-form').submit(function (evt) {
            evt.preventDefault();
            $('#record-import-form-submit').html("<img src=\"{{ asset('public/assets/dashboard/images/loading.gif') }}\" style=\"height: 20px\"/> {!! __('backend.add') !!}");
            $('#record-import-form-submit').prop('disabled', true);
                ...
                success: function (result) {
                    ...
                    if (result.stat === 'success') {
                        if (result.step === 2) {
                            ...
                            $("#fields_import_process").html("<div class='p-a m-b text-center h6'><img src=\"{{ asset('public/assets/dashboard/images/loading.gif') }}\" style=\"height: 25px\"/> {!! __('backend.importLoading') !!}</div>");
